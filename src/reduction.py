from settings import DOCKER, FLATTER_HOST_DIR, FLATTER_MODE  # type: ignore #noqa
...
def flatter(basis, mode=3, docker=DOCKER):
    basis_s = sage_to_fplll_matrix(basis, nrows=len(list(basis)))

    if mode == 0:
        # run reduction.py from host, flatter by invoking docker
        logger.info(f"flatter run from host by invoking docker (mode {mode})")
        cmd = [
            "bash",
            "-c",
            # single-core flatter
            f'{docker} run --platform linux/amd64 -v $(pwd):/mnt -e OMP_NUM_THREADS=\'1\' -i sparse-farce-image flatter'
        ]
        env = None
    elif mode == 1:
